...
class Database implements I\IDatabase {
    ...
    protected function apply( $method, $args ){
        ...
        try {
            if( empty( $args ))
                throw new \Players\Exception\Database\QueryLogicError( 'No arguments in '.__CLASS__.'::'. $method );
            if(!is_array( $args ))
                throw new \Players\Exception\Database\QueryLogicError( 'Wrong arguments in '.__CLASS__.'::'. $method );

            $tpl = array_shift( $args );
            if(!is_string( $tpl ))
                throw new \Players\Exception\Database\QueryLogicError( 'SQL template not found in '.__CLASS__.'::'. $method );
        ...
        catch( \Players\Exception\Database\Connection $E ){
            if( $this->log )
                \Players\System\Registry::instance()->Logger()->log( 'sql-connection-errors', $E->getMessage());
            return false;
        }
        catch( \Players\Exception\Database\SQLQuery $E ){
            if( $this->log )
                \Players\System\Registry::instance()->Logger()->log( 'sql-errors', $E->getMessage() .' in query '."\n". $this->lastQuery());
            return false;
        }
        catch( \Exception $E ){
            throw $E;
